...
async def on_ready():
   ...
   for filename in os.listdir('./cogs'):
    ...
    #Changes application presence
    await bot.change_presence(activity=discord.Game(name="play.thegathering.net"))
...
async def update_staff_info(guild):
    global staff_info
    total_count = 0
    for role in guild.roles:
        if role.name in staff_roles:
            count = len(role.members)
            staff_info[role.name]['count'] = count
            staff_info[role.name]['members'] = [member.mention for member in role.members]
            total_count += count
    return total_count
...
async def stafflist(ctx):
    ...
    global staff_info
    total_count = await update_staff_info(ctx.guild)
    ...
    for role in staff_roles:
        count = staff_info[role]['count']
        members = '\n'.join(staff_info[role]['members'])
